...
const ChatInterface = () => {
  ...
  const URL = "https://3.38.149.152.nip.io/stomp/content";

  useEffect(() => {
    axios.get(`/api/messages/member?page=${page}`)
        .then((response) => {
  ...
  const fetchMembers = async (roomId) => {
    try {
      const response = await axios.get(`/api/messages/room/${roomId}/members`);
  ...
  const fetchAndSetMessages = () => {
    if (selectedChatRoom !== null) {
      axios.get(`/api/messages/rooms/member/${selectedChatRoom}`)
          .then(response => {
